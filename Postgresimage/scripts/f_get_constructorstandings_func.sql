CREATE OR REPLACE FUNCTION f1_data.f_get_f1_constructorstandings() RETURNS VOID AS $$
	DECLARE

			v_race_rec RECORD;
			v_url constant text := 'http://ergast.com/api/f1/{YEAR}/constructorStandings.json?limit=1000';
			v_stmt text;
			v_count int := 0;
	...
	BEGIN
			...
			LOOP
				...
				IF v_count < 1 THEN
					...
					INSERT INTO f1_data.f1_constructorstandings_json (fetched_at,season,constructorstandings)
